 ...
 DAO ins=new DAO();
   ...
   age.setText("");
    ...
    try
    {
      ...
      if(ins.test(pseudo))ch+="pseudo existe \n";
      if(validationEmail(email)==false)ch+="mail invalide \n";
      if(ins.testMail(email,1)==true )ch+="mail existe";
      if(ch.equals("")){ins.insert(pseudo,name, pren, email,passe ,tel, ciin, agee, quest,selected);
  ...
  private boolean validationEmail( String email )   {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
       List<String> depots=ins.DisplayAllDepots();
